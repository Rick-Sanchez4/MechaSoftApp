...
using MechaSoft.Application.CQ.Dashboard.Queries.GetMechanicDashboardStats;
...
public static class DashboardEndpoints
{
    public static void RegisterDashboardEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        // GET /api/dashboard/mechanic/{mechanicId} - Mechanic dashboard stats (my orders only, no revenue)
        dashboard.MapGet("/mechanic/{mechanicId:guid}", Queries.GetMechanicDashboardStats);
    ...
    private static class Queries
    {
        ...
        public static async Task<Results<Ok<MechanicDashboardStatsResponse>, BadRequest<Error>>> GetMechanicDashboardStats(
            [FromRoute] Guid mechanicId,
            [FromServices] ISender sender)
        {
            var query = new GetMechanicDashboardStatsQuery(mechanicId);
            var result = await sender.Send(query);
