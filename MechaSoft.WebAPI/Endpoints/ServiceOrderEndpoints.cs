...
using MechaSoft.Application.CQ.ServiceOrders.Commands.CancelServiceOrder;
...
public static class ServiceOrderEndpoints
{
    ...
    {
        var serviceOrders = routes.MapGroup("api/service-orders").WithTags("Service Orders").RequireAuthorization();
        ...
        // PATCH /api/service-orders/{id}/cancel - Cancel order (returns parts to stock)
        serviceOrders.MapPatch("/{id:guid}/cancel", Commands.CancelOrder);
    }
    ...
    private static class Commands
    {
        ...
        public static async Task<Results<Ok<CancelServiceOrderResponse>, BadRequest<Error>>> CancelOrder(
            [FromServices] ISender sender,
            Guid id,
            [FromBody] CancelOrderRequest request)
        {
            ...
            var command = new CancelServiceOrderCommand(id, request.Reason);
            var result = await sender.Send(command);
