...
public class GetVehiclesQueryHandler(IUnitOfWork unitOfWork) : IRequestHandler<GetVehiclesQuery, Result<GetVehiclesResponse, Success, Error>>
{
    public async Task<Result<GetVehiclesResponse, Success, Error>> Handle(GetVehiclesQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            // Get all vehicles
            var allVehicles = await unitOfWork.VehicleRepository.GetAllAsync();
